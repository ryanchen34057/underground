...
public class LevelHandler implements HandlerInterface {
    ...
    public LevelHandler(Camera cam) {
        this.cam = cam;
...
//                    player = new Player(x * 64 + 35, y * 64 + 35, Player.WIDTH, Player.HEIGHT, Id.player);
    ...
    public Dimension getBluePortalCor() { return bluePortalCor; }

    private boolean inTheScreen(Tile t) {
        return (t.getX() >= cam.getX() - (Game.WIDTH * Game.SCALE)) && (t.getX() < cam.getX() + (Game.WIDTH * Game.SCALE))
                && (t.getY() >= cam.getY() - (Game.HEIGHT * Game.SCALE)) && (t.getY() < cam.getY() + (Game.HEIGHT * Game.SCALE));
    }
}
